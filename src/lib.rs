...
mod test {
                     ...
                     utility_with_global_mutex(|| {
                            ...
                            // test 0, preamble: verify that base config *does* cause changes to files/dirs
                            {
                                   ...
                                   if directory_before_state == directory_after_state {
                                          tracing::error!("PREAMBLE TEST FAILED: base_args should have caused changes but didn't");
       ...
       mod tests_snapshot {
              ...
              fn utility_capture_directory_tree_as_string(path: &str) -> Result<String> {
                     ...
                     entries.sort();
                     let formatted = entries.iter().map(|p| p.to_string_lossy().to_string()).collect::<Vec<_>>().join("\n");
                     Ok(formatted)
              }
